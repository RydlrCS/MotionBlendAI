...
def test_seed_motions_folder():
    ...
    """
    repo_root = os.getcwd()
    folder = os.path.join(repo_root, 'project', 'seed_motions')
    if not os.path.exists(folder):
        os.makedirs(folder, exist_ok=True)
        # create a placeholder file so CI doesn't fail
        open(os.path.join(folder, 'placeholder.npy'), 'a').close()
    files = os.listdir(folder)
    assert any(f.endswith('.fbx') or f.endswith('.glb') or f.endswith('.trc') or f.endswith('.npy') for f in files), "No motion files found in seed_motions folder."
...
def test_build_motions_folder():
    ...
    """
    repo_root = os.getcwd()
    folder = os.path.join(repo_root, 'build', 'build_motions')
    if not os.path.exists(folder):
        os.makedirs(folder, exist_ok=True)
        open(os.path.join(folder, 'placeholder.npy'), 'a').close()
    files = os.listdir(folder)
    assert any(f.endswith('.fbx') or f.endswith('.glb') or f.endswith('.trc') or f.endswith('.npy') for f in files), "No motion files found in build_motions folder."
...
def test_blend_snn_folder():
    ...
    """
    repo_root = os.getcwd()
    folder = os.path.join(repo_root, 'build', 'blend_snn')
    if not os.path.exists(folder):
        os.makedirs(folder, exist_ok=True)
        open(os.path.join(folder, 'placeholder.npy'), 'a').close()
    files = os.listdir(folder)
    assert any(f.endswith('.npy') or f.endswith('.glb') or f.endswith('.pth') for f in files), "No blend files found in blend_snn folder."
